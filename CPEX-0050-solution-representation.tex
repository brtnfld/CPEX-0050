...
\item[Isogeometric analysis ($C^k$, $k \geq 1$)] uses basis functions
  ...
  interpolation metadata (CPEX~0045), not in the representation type.
\end{description}

The \texttt{SolutionRepresentationUserDefined} value remains available
for unforeseen cases, and additional enumeration values can be added
in future CPEXes without breaking backward compatibility.
...
(SIDS Section~9.4).  \texttt{ZoneSubRegion\_t} holds
...
\begin{sidsnode}{ZoneSubRegion\_t (amended, SIDS Section 9.4)}
...
/*
 * Write/read the solution representation for a ZoneSubRegion_t node.
 * Same semantics as the FlowSolution_t counterparts.
 *
 * fn   - file number
 * B    - base index
 ...
 * type - SolutionRepresentation_t value
 */
int cg_subreg_representation_write(
    int fn, int B, int Z, int SR,
    CGNS_ENUMT(SolutionRepresentation_t) type);

int cg_subreg_representation_read(
    int fn, int B, int Z, int SR,
    CGNS_ENUMT(SolutionRepresentation_t) *type);
\end{lstlisting}
...
! ZoneSubRegion_t
subroutine cg_subreg_representation_write_f( &
    fn, B, Z, SR, type, ier)
  integer, intent(in) :: fn, B, Z, SR, type
  integer, intent(out) :: ier

subroutine cg_subreg_representation_read_f( &
    fn, B, Z, SR, type, ier)
...
typedef struct {            /* FlowSolution_t node */
    ...
    double representation_id;   /* CGIO node id for overwrite/delete */
} cgns_sol;
...
The \texttt{representation} field defaults to
\texttt{SolutionRepresentationNull} (value~0), which is the
zero-initialization value and the ``not specified'' state.  The
\texttt{representation\_id} field stores the CGIO node identifier so
...
\begin{lstlisting}[language=C]
const char *SolutionRepresentationName[NofValidSolutionRepresentation] = {
    "Null",
    "UserDefined",
    "Continuous",
    "Discontinuous"
...
nodes with label \texttt{SolutionRepresentation\_t} during the
...
\item During memory allocation of \texttt{cgns\_sol},
  ...
  \texttt{representation} field is zero-initialized, which corresponds
  to \texttt{SolutionRepresentationNull}.
\item During the child-node traversal, if a child with label
  \texttt{"SolutionRepresentation\_t"} is found, read its C1 string
  data and map it to the enum value via
  \texttt{cgi\_enum\_val("SolutionRepresentation\_t", string\_data,
  \&enum\_val)}.  Store the result in the struct's
  \texttt{representation} field.
\item If no such child node exists, the field retains its default
  \texttt{SolutionRepresentationNull} value.
\end{enumerate}
...
\texttt{cg\_sol\_representation\_read}) return the cached struct field
...
\texttt{SolutionRepresentationNull}, the read function simply returns
the struct value and \texttt{CG\_OK}:

\begin{lstlisting}[language=C]
int cg_sol_representation_read(int fn, int B, int Z, int S,
                               CGNS_ENUMT(SolutionRepresentation_t) *type) {
    cgns_sol *sol;
...
solutions and queries the representation will always receive
\texttt{CG\_OK} with \texttt{SolutionRepresentationNull} for files
that predate this CPEX.

\subsubsection{Write Function Implementation}

The \texttt{\_write} functions create (or overwrite) the
\texttt{SolutionRepresentation} child node.  Writing
\texttt{SolutionRepresentationNull} removes the node if present,
...
\begin{lstlisting}[language=C]
int cg_sol_representation_write(int fn, int B, int Z, int S,
                                CGNS_ENUMT(SolutionRepresentation_t) type) {
    cgns_sol *sol;
    cgsize_t length;
    double posit_id;
    ...
    /* Validate enum value */
    if (type < 0 || type >= NofValidSolutionRepresentation) {
        cgi_error("Invalid SolutionRepresentation value: %d", type);
        return CG_ERROR;
    }

    /* Delete existing node if writing Null */
    if (type == CGNS_ENUMV(SolutionRepresentationNull)) {
        if (sol->representation !=
                CGNS_ENUMV(SolutionRepresentationNull)) {
            if (cgi_delete_node(sol->id,
                    sol->representation_id))
                return CG_ERROR;
            sol->representation =
                CGNS_ENUMV(SolutionRepresentationNull);
            sol->representation_id = 0;
        }
        return CG_OK;
    }
    ...
    length = (cgsize_t)strlen(
        SolutionRepresentationName[type]);
    if (cgi_new_node(sol->id, "SolutionRepresentation",
            "SolutionRepresentation_t", &posit_id,
            "C1", 1, &length,
            (void *)SolutionRepresentationName[type]))
        return CG_ERROR;

    sol->representation = type;
    sol->representation_id = posit_id;
    return CG_OK;
}
\end{lstlisting}

\textbf{Note:} The \texttt{representation\_id} field (a \texttt{double}
storing the CGIO node identifier) must be added to the internal
structures alongside the \texttt{representation} field so that the
...
\texttt{SolutionRepresentation\_t = Discontinuous} and the solution
...
\item \textbf{ZoneSubRegion\_t amendment:} The
  \texttt{ZoneSubRegion\_t} node definition (SIDS Section~9.4) is
  similarly amended.
...
\item \textbf{Scope: \texttt{BCData\_t} is excluded.}
  \texttt{SolutionRepresentation\_t} is \emph{not} added to
  ...
  follows from the solution representation on the parent
  \texttt{FlowSolution\_t} node.  Adding
  \texttt{SolutionRepresentation\_t} to \texttt{BCData\_t} would be
  redundant and would introduce a potential inconsistency if the BC
  representation differs from the solution it constrains.  Should a
  future use case require per-BC representation metadata (e.g.,
  coupling frameworks imposing boundary data in a different
  representation than the interior), it can be proposed as a
  separate, narrowly scoped CPEX without affecting this proposal.

\item \textbf{Per-solution-node granularity (not per-field).}
  The representation applies at the \texttt{FlowSolution\_t} level,
  meaning all \texttt{DataArray\_t} children share the same
  representation.  This follows the precedent set by
  ...
  representations within a single zone (e.g., HDG methods with
...
\item \textbf{\texttt{CellCenter} + \texttt{Continuous}: permitted
  ...
  \texttt{SolutionRepresentation\_t} and \texttt{GridLocation\_t}.
  ...
  \textbf{Convention:} \texttt{SolutionRepresentation\_t = Continuous}
...
\midrule
\endhead
\texttt{SolutionRepresentation\_t} & New enumeration type: \texttt{Null}, \texttt{UserDefined}, \texttt{Continuous}, \texttt{Discontinuous}. \\
...
\texttt{cg\_subreg\_representation\_write} & C: write solution representation for a \texttt{ZoneSubRegion\_t} node. \\
\texttt{cg\_subreg\_representation\_read} & C: read solution representation for a \texttt{ZoneSubRegion\_t} node. \\
...
\texttt{cg\_subreg\_representation\_write\_f} & Fortran: write solution representation (\texttt{ZoneSubRegion\_t}). \\
\texttt{cg\_subreg\_representation\_read\_f} & Fortran: read solution representation (\texttt{ZoneSubRegion\_t}). \\
\bottomrule
\end{longtable}
\end{center}
