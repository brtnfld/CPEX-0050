...
int cg_sol_representation_read(int fn, int B, int Z, int S,
                               CGNS_ENUMT(SolutionRepresentation_t) *type) {
    cgns_sol *sol = cgi_get_sol(fn, B, Z, S);
...
\item \textbf{Scope: \texttt{BCData\_t}?}
  Should \texttt{SolutionRepresentation\_t} also be permitted on
  \texttt{BCData\_t}?  The current proposal covers
  \texttt{FlowSolution\_t}, \texttt{DiscreteData\_t}, and
  \texttt{ZoneSubRegion\_t}; the committee may wish to extend further.
...
\item \textbf{Interaction with \texttt{GridLocation\_t = CellCenter}.}
  ...
  the library enforce consistency (reject \texttt{Continuous}
  with \texttt{CellCenter}), or
